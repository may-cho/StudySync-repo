...
def approve_group_request(request,group_id):
    ...
    TimetableSlot(
        ...
        custom_name=f"Group study: {group.name}"
    ) for day in days   
    ]
...
def get_group_data(request,group_id) :
     ...
     invitations = GroupInvitation.objects.filter(group=group,is_admin_approved=False).select_related('group','invited_by','invited_student')
